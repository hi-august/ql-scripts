    ...
    for (var o of taskList) {
        switch (o.title) {
            ...
            case "CX11001": //查询企业
                console.log("开始任务：" + oo[o.title]);
                await get(
                    `s/getHeadBrandAndPersonAjax?q=${encodeURI(rand())}`,
                    "get"
                );
            ...
            case "CX11002": //查询老板
                console.log("开始任务：" + oo[o.title]);
                await get(
                    `person/relevantPersonalAjax?page=1&q=${encodeURI(
                        rand()
                    )}&size=10`,
                    "get"
                );
            ...
            case "CX11004": //查询商标
                console.log("开始任务：" + oo[o.title]);
                await get(
                    `c/markproAjax?q=${encodeURI(
                        rand()
                    )}&p=1&s=10&f=%7B%7D&o=%7B%7D`,
                    "get"
                );
                await sleep(radomTimers());
                break;
      ...
      */
            case "CX12001": //添加监控
                console.log("开始任务：" + oo[o.title]);
                for (id of [
                    29829264524016, 28696417032417, 31370200772422,
                    31242153386614,
                ]) {
                ...
                await get(
                    `zxcenter/cancelMonitorAjax?pid=29710155220353`,
                    "get"
                );
                await sleep(radomTimers());
                break;
        ...
        for (a = 0; a < aqcookieArr.length; a++) {
            ...
            if (logininfo.data.isLogin == 1) {
                ...
                for (task of claimList) {
                    ...
                    if (clres.status == 0)
                        console.log(
                            `领取成功！获得${clres.data.totalScore}爱豆r\n`
                        );
                }
                console.log("去查询爱豆积分");
