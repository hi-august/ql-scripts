...
let jzreadurlArr = [],
    jzreadcount = "";
let jzreadurl = $.isNode()
    ? process.env.jzreadurl
        ? process.env.jzreadurl
        : ""
    : $.getdata("jzreadurl")
    ? $.getdata("jzreadurl")
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!$.isNode()) {
            ...
            if (
                process.env.jzreadurl &&
                process.env.jzreadurl.indexOf("@") > -1
            ) {
...
function jzreadck() {
    if ($request.url.indexOf("info") > -1) {
        ...
        if (jzreadurl) $.setdata(jzreadurl, `jzreadurl${status}`);
    ...
    return new Promise((resolve) => {
        ...
        let nm = {
            ...
            headers: {
                ...
                "User-Agent":
                    "ChapterNine/1.2.3 (com.ass.jiuzhang; build:1120; iOS 14.3.0) Alamofire/5.4.2",
                "Accept-Language":
                    "zh-Hans-CN;q=1.0, zh-Hant-HK;q=0.9, zh-Hant-CN;q=0.8",
            },
        };
        $.get(nm, async (error, response, data) => {
            try {
                ...
                if (result.code == 0)
                    $.log(
                        "can_receive：" +
                            result.data["can_receive"] +
                            "\nis_receive：" +
                            result.data["is_receive"] +
                            "\nis_video：" +
                            result.data["is_video"] +
                            "\nseconds：" +
                            result.data.seconds
                    );
    ...
    return new Promise((resolve) => {
        ...
        $.post(nm, async (error, response, data) => {
            try {
                const result = JSON.parse(data);
                if (logs) $.log(data);
                if (result.code == 0) $.log("coin：" + result.data.coin);
                ...
                if (result.code == 0)
                    $.log(
                        "invite_code：" +
                            ...
                            "\nmy coin：" +
                            result.data.integral +
                            "\nwork_money：" +
                            result.data["work_money"] +
                            "\nmoney：" +
                            result.data.money
                    );
    ...
    return new Promise((resolve) => {
        ...
        $.get(nm, async (error, response, data) => {
            try {
                ...
                //$.log(data)
                if (logs) $.log(data);
                if (result.code == 0) readlist = result.data.list;
...
async function read(id) {
    return new Promise((resolve) => {
        ...
        let nm = {
            url:
                `https://api.st615.com/v1/comment/list?&article_id=${id}&page=1&limit=10&token=` +
                token,
            //body:  `token=`+token,
            headers: {
                Host: "api.st615.com",
    ...
    return new Promise((resolve) => {
        ...
        $.post(nm, async (error, response, data) => {
            ...
            try {
                ...
                if (result.code != 0) $.log("error this id have read");
            } catch (e) {
                $.logErr(e, response);
            } finally {
                resolve();
...
async function getvideo() {
    return new Promise((resolve) => {
        ...
        let nm = {
            url:
                `https://api.st615.com/v1/article/list?type=2&cid=35&page=1&terminal=Apple&version=1.2.3&token=` +
                token,
            //body:  `token=`+token,
            headers: {
                Host: "api.st615.com",
        ...
        $.get(nm, async (error, response, data) => {
            ...
            try {
                const result = JSON.parse(data);
                if (logs) $.log(data);
                if (result.code == 0) a = result.data.list;
...
async function share() {
    return new Promise((resolve) => {
        ...
        let nm = {
            ...
            body:
                `device=iPhone%208%20Plus&id=&os=14.4&source=article&token=` +
                token,
            headers: {
                Host: "api.st615.com",
                "Content-Type":
