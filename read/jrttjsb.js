...
let userAgent =
    ...
    "Dalvik/2.1.0 (Linux; U; Android 7.1.2; VOG-AL10 Build/HUAWEIVOG-AL10) NewsArticle/8.2.8 tt-ok/3.10.0.2";
let userAgentArr = [];
let userHeader =
    ($.isNode() ? process.env.jrttjsbHeader : $.getdata("jrttjsbHeader")) || "";
let userHeaderArr = [];
let jrttjsbFarm =
    ($.isNode() ? process.env.jrttjsbFarm : $.getdata("jrttjsbFarm")) || 0;

let userIdx = 0;
let UAcount = 0;
let userStatus = [];
let maxReadPerRun =
    ($.isNode() ? process.env.jrttjsbReadNum : $.getdata("jrttjsbReadNum")) ||
    10;
let readList = [];

let validList = [];
let adIdList = [
    26, 181, 186, 187, 188, 189, 190, 195, 210, 214, 216, 225, 308, 324, 327,
    329,
];
    ...
    if ($request.url.indexOf("luckycat/lite/v1/task/page_data") > -1) {
        ...
        if (userHeader) {
            if (userHeader.indexOf(userCK) == -1) {
                ...
                $.msg(
                    jsname +
                        ` 获取第${ckList.length}个jrttjsbHeader成功: ${userCK}`
                );
            }
        } else {
...
async function ReadArtsReward() {
    ...
    if (result.err_no == 0) {
        console.log(
            `用户${userIdx + 1}阅读文章获得${
                result.data.score_amount
            }金币，今日阅读总收入：${result.data.total_score_amount}金币`
        );
    } else {
...
async function DailyArtsReward() {
    ...
    if (result.err_no == 0) {
        console.log(
            `用户${userIdx + 1}领取每日阅读奖励获得${
                result.data.score_amount
            }金币`
        );
    } else {
        console.log(
            `用户${userIdx + 1}领取每日阅读奖励失败：${result.err_tips}`
        );
    }
}
...
async function DailyPushReward() {
    ...
    if (result.err_no == 0) {
        console.log(
            `用户${userIdx + 1}领取每日推送奖励获得${
                result.data.score_amount
            }金币`
        );
    } else {
        console.log(
            `用户${userIdx + 1}领取每日推送奖励失败：${result.err_tips}`
        );
    }
}
    ...
    if (result.err_no == 0) {
        ...
        if (doTask == 0) {
            ...
            if (
                ...
                result.data.treasure.current_time
            ) {
...
async function SignIn() {
    ...
    if (result.err_no == 0) {
        console.log(
            `用户${userIdx + 1}签到成功，获得${
                result.data.score_amount
            }金币，已连续签到${result.data.sign_times}天`
        );
    } else {
...
async function OpenTreasureBox() {
    ...
    if (result.err_no == 0) {
        console.log(
            `用户${userIdx + 1}开宝箱获得${result.data.score_amount}金币`
        );
    } else {
...
async function ExcitationAd(task_id) {
    ...
    if (result.err_no == 0) {
        ...
        console.log(
            `用户${userIdx + 1}看视频任务[${task_id}]失败：${result.err_tips}`
        );
...
async function GetWalkBonus() {
    ...
    if (result.err_no == 0) {
        console.log(
            `用户${userIdx + 1}领取走路奖励获得${result.data.score_amount}金币`
        );
    } else {
...
async function DoneEat() {
    ...
    if (result.err_no == 0) {
        console.log(
            `用户${userIdx + 1}领取吃饭补贴获得${result.data.score_amount}金币`
        );
    } else {
    ...
    if (result.err_no == 0) {
        ...
        if (result.data.sleeping == true) {
            if (sleepHour >= 12) {
                ...
                result.data.sleep_unexchanged_score == result.data.max_coin &&
                curHour >= 7
            ) {
                let rnd = Math.random();
                if (rnd > 0.95) {
                    ...
                    console.log(
                        `用户${
                            userIdx + 1
                        }随机醒来时间，本次不进行醒来，已经睡了${sleepHour}小时，可以获得${
                            result.data.sleep_unexchanged_score
                        }金币`
                    );
                }
            } else {
                console.log(
                    `用户${
                        userIdx + 1
                    }睡眠中，已经睡了${sleepHour}小时，可以获得${
                        result.data.sleep_unexchanged_score
                    }金币，上限${result.data.max_coin}金币`
                );
            }
        } else {
...
async function SleepDone(amount) {
    ...
    if (result.err_no == 0) {
        ...
        console.log(
            `用户${userIdx + 1}领取睡觉金币奖励失败：${result.err_tips}`
        );
    }
}
...
async function FarmOfflineDouble() {
    ...
    if (result.status_code == 0) {
        ...
        console.log(
            `用户${userIdx + 1}农场离线产量翻倍失败：${result.message}`
        );
    }
}
...
async function RewardFarmThreeGift(gift_id) {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}领取农场三餐礼包获得${
                result.data.reward_num
            }水滴`
        );
    } else {
        console.log(
            `用户${userIdx + 1}领取农场三餐礼包失败：${result.message}`
        );
    }
}
...
async function QueryFarmThreeGift() {
    ...
    if (result.status_code == 0) {
        ...
        console.log(
            `用户${userIdx + 1}查询农场三餐礼包状态失败：${result.message}`
        );
    }
}
...
async function QueryFarmTask() {
    ...
    if (result.status_code == 0) {
        ...
        console.log(
            `用户${userIdx + 1}查询农场任务列表失败：${result.message}`
        );
    }
}
...
async function RewardFarmTask(id) {
    ...
    if (result.status_code == 0) {
        ...
        console.log(
            `用户${userIdx + 1}领取农场任务奖励[task_id=${
                result.data.task_id
            }]获得${result.data.reward_num}${typeStr}，剩余${typeStr}数量${
                result.data.current_num
            }`
        );
    } else {
        console.log(
            `用户${userIdx + 1}领取农场任务奖励失败：${result.message}`
        );
    }
}
...
async function FarmWater() {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}农场浇水成功，剩余水滴：${result.data.water}`
        );
...
async function FarmOpenBox() {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}开农场宝箱获得${result.data.incr_coin}金币`
        );
        if (result.data.excitation_ad_score_amount > 0)
            await FarmOpenBoxVideo();
    } else {
...
async function FarmOpenBoxVideo() {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}看农场宝箱视频获得${result.data.incr_coin}金币`
        );
    } else {
...
async function FarmSign() {
    ...
    if (result.status_code == 0) {
        ...
        console.log(
            `用户${userIdx + 1}签到获得${
                result.data.reward_num
            }${str}，剩余${str}数量${result.data.cur_reward_num}`
        );
    } else {
...
async function FarmUnlock(land_id) {
    ...
    if (result.status_code == 0) {
        ...
        console.log(
            `用户${userIdx + 1}解锁${land_id}号土地失败：${result.message}`
        );
    }
}
...
async function QueryTreeSignStatus() {
    ...
    if (result.status_code == 0) {
        ...
        console.log(
            `用户${userIdx + 1}查询种树签到状态失败：${result.message}`
        );
    }
}
...
async function TreeSign() {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}种树签到获得${result.data.reward_item.num}${
                result.data.reward_item.name
            }`
        );
    } else {
...
async function TreeChallengeChoose(id) {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}选择浇水${result.data.red_point.times}次挑战`
        );
    } else {
...
async function TreeChallengeReward() {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}领取浇水挑战奖励获得${
                result.data.reward_item.num
            }水滴`
        );
    } else {
        console.log(
            `用户${userIdx + 1}领取浇水挑战奖励失败：${result.message}`
        );
    }
}
...
async function RewardTreeThreeGift(task_id) {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}领取种树三餐礼包获得${
                result.data.reward_item.num
            }${result.data.reward_item.name}`
        );
    } else {
        console.log(
            `用户${userIdx + 1}领取种树三餐礼包失败：${result.message}`
        );
    }
}
...
async function QueryTreeThreeGift() {
    ...
    if (result.status_code == 0) {
        for (let item of result.data.gift_list) {
            if (
                item.rounds == 1 &&
                curHour >= item.available_time.begin &&
                curHour < item.available_time.end
            ) {
        ...
        console.log(
            `用户${userIdx + 1}查询种树三餐礼包状态失败：${result.message}`
        );
    }
}
...
async function QueryTreeWaterTask() {
    ...
    if (result.status_code == 0) {
        ...
        console.log(
            `用户${userIdx + 1}查询种树水滴任务列表失败：${result.message}`
        );
    }
}
...
async function TreeWaterReward(task_id) {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}领取水滴任务[id=${task_id}]获得${
                result.data.reward_item.num
            }${result.data.reward_item.name}`
        );
    } else {
        console.log(
            `用户${userIdx + 1}领取水滴任务[id=${task_id}]失败：${
                result.message
            }`
        );
    }
...
async function TreeWater() {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}种树浇水成功，剩余水滴：${
                result.data.kettle.water_num
            }`
        );
...
async function QueryTreeStatus() {
    ...
    if (result.status_code == 0) {
        if (
            result.data.red_points.challenge &&
            result.data.red_points.challenge.state == 4
        ) {
        ...
        if (
            result.data.red_points.box &&
            result.data.red_points.box.rounds > 0 &&
            result.data.red_points.box.state == 4
        ) {
            await TreeOpenBox();
        }
        if (result.data.kettle.water_num >= 100) {
...
async function RewardTreeWaterBottle() {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}领取种树水瓶奖励获得${
                result.data.reward_item.num
            }水滴`
        );
    } else {
        console.log(
            `用户${userIdx + 1}领取种树水瓶奖励失败：${result.message}`
        );
    }
}
...
async function TreeOpenBox() {
    ...
    if (result.status_code == 0) {
        console.log(
            `用户${userIdx + 1}种树开宝箱获得${result.data.incr_coin}金币`
        );
    } else {
...
function safeGet(data, caller) {
    try {
        ...
        console.log(e);
        console.log(
            `Function ${caller}: 服务器访问数据为空，请检查自身设备网络情况`
        );
        return false;
    }
}
